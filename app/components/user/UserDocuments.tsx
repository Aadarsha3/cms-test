...
export interface UserDocumentsProps {
  user: UserDetail & { documents?: UserDocument[] };
  isEditing?: boolean;
}

export function UserDocuments({ user, isEditing = false }: UserDocumentsProps): React.JSX.Element {
  ...
  const handleDocumentUpload = async (e: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      console.error("Document upload failed:", axiosError);
      toast({
        title: "Upload failed",
        description: axiosError.response?.data?.detail || axiosError.message || "Failed to upload document",
        variant: "destructive",
      });
    } finally {
      setUploading(false);

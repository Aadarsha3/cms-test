...
export function UpcomingEvents({ 
    ...
    const displayedEvents = hideShowMore
        ...
        : showAll ? allUpcomingEvents : allUpcomingEvents.slice(0, maxEvents);
                        ...
                        {displayedEvents.map((event, index) => (
                            ...
                            >
                                <div className="flex flex-col items-center bg-white dark:bg-zinc-950 rounded-lg py-1 px-2 min-w-[3rem] text-center shadow-sm border group-hover:border-primary/30 transition-all leading-tight">
                                    <span className="text-[9px] font-bold uppercase text-primary/70 mb-0.5">{format(event.date, "MMM")}</span>
                                    <span className="text-sm font-black tracking-tighter mt-[-2px]">{format(event.date, "d")}</span>
                                </div>

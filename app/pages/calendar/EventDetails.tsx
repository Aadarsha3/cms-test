...
export function EventDetails({ date, events, isAdmin, onEdit, onDelete }: EventDetailsProps) {
    if (!date) return null;
            ...
            <CardHeader className="py-2.5 px-4 border-b border-primary/5 bg-primary/[0.01]">
                <div className="flex items-center justify-between">
                    ...
                    {selectedEvents.length > 0 && (
                        <Badge variant="outline" className="text-[10px] h-5 bg-primary/5 border-primary/10">
                ...
                {selectedEvents.length > 0 ? (
                    <div className="divide-y divide-primary/5">
                        {selectedEvents.map(event => (
                            <div key={event.id} className="group p-3 hover:bg-white dark:hover:bg-zinc-800/50 transition-all">
                    ...
                    <div className="flex flex-col items-center justify-center p-4 min-h-[70px] text-center">
                        ...
                        <p className="text-xs font-medium text-muted-foreground/40">No events scheduled</p>
                    </div>
                )}
            </CardContent>
        </Card>

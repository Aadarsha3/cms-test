...
export function StudentTable() {
  ...
  const fetchUsernames = async (studentList: StudentResponse[]) => {
    const ids = studentList.map((s) => s.userId).filter(Boolean);
    const results = await Promise.allSettled(
      ids.map((id) => userApi.get(`/users/${id}`))
    );
    const map: Record<string, string> = {};
    results.forEach((result, i) => {
      if (result.status === "fulfilled" && result.value.data?.username) {
        map[ids[i]] = result.value.data.username;
      }
    });
    setUsernameMap(map);
  };
                ...
                {loading ? (
                  ...
                  filteredStudents.map((student, index) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {student.userId && usernameMap[student.userId]

    ...
    <MainLayout title="My Profile">
      <div className="max-w-4xl mx-auto space-y-6 pb-10">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setLocation("/change-password")} size="sm" variant="outline" className="gap-2 shadow-sm">
        ...
        {roleData && (
          <>
            <Card>
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <InfoField label="Email Address" value={roleData.email || accountData?.primaryEmail || authUser.email} icon={Mail} />
                  <PhoneInputField label="Phone Number" value={roleData.phoneNumber || ""} onChange={() => {}} disabled />
                  <DatePickerField label="Date of Birth" value={roleData.dateOfBirth || ""} onChange={() => {}} disabled />
                  <InfoField label="Gender" value={roleData.gender || "N/A"} icon={User} />
                  <InfoField label="Address" value={roleData.address || roleData.presentAddress || "N/A"} icon={MapPin} />
                </div>
              </CardContent>
            </Card>
            ...
            {(isStudent || isStaffStaff) && (
              <Card>
                <CardHeader><CardTitle className="text-lg">{isStudent ? "Guardian Information" : "Employment Details"}</CardTitle></CardHeader>
                <CardContent>
                  {isStudent ? (
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      <InfoField label="Guardian Name" value={roleData.guardianName || "N/A"} icon={User} />
                      <PhoneInputField label="Contact Number" value={roleData.guardianPhoneNumber || ""} onChange={() => {}} disabled />
                      <InfoField label="Relationship" value={roleData.guardianRelation || "N/A"} icon={Heart} />
                    </div>
                  ) : (
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      <DatePickerField label="Join Date" value={roleData.joinDate || ""} onChange={() => {}} disabled />
                      <DatePickerField label="Termination Date" value={roleData.terminationDate || ""} onChange={() => {}} disabled />
                    </div>
                  )}
                </CardContent>
              </Card>
        ...
        {roleNotFound && !roleData && (
          <Card className="border-border bg-muted/30">
            <CardContent className="p-6 text-center text-muted-foreground space-y-1">
              ...
              <p className="text-sm">Your {isStudent ? 'student' : 'staff'} record has not been created by an administrator yet.</p>
            </CardContent>
          </Card>
        )}
      </div>

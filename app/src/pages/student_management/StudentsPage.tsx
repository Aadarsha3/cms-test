...
export function StudentsPage() {
    const [apiStudents, setApiStudents] = useState<StudentResponse[]>([]);
    ...
    const fetchStudents = async () => {
        ...
        setError(null);
        try {
            const response = await userApi.get<StudentResponse[]>("/users", {
                params: {
                    page,
                    size,
                    sort,
    ...
    const filteredStudents = apiStudents.filter((s) => {
        ...
        const email = s.primaryEmail?.toLowerCase() || "";
        const id = s.id?.toLowerCase() || "";
        const givenName = s.givenName?.toLowerCase() || "";
        const familyName = s.familyName?.toLowerCase() || "";
                                ...
                                {loading ? (
                                    ...
                                    filteredStudents.map((student, index) => (
                                        <TableRow
                                            ...
                                            onClick={() =>
                                                student.id && openStudentDetails(student.id)
                                            }
                                        >
                                            ...
                                            <TableCell>{student.primaryEmail || "N/A"}</TableCell>
                                            <TableCell>
                                                {student.createdAt
                ...
                {!loading && apiStudents.length > 0 && (
                    <div className="flex items-center justify-between">
                        <div className="text-sm text-muted-foreground">
                            Showing {page * size + 1}-{page * size + apiStudents.length}{" "}
                            students
                        </div>
                        <div className="flex gap-2">
                            ...
                            <Button
                                ...
                                disabled={apiStudents.length < size}
                            >
                                Next
                            </Button>
                        </div>
